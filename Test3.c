...
struct song e;//e struct pointer
...
// will be 0, 0
COORD cord = { 0, 0 };//pointer

// function to set the
// coordinates
void gotoxy(int x, int y)//x co-ordinate,y co-ordinate
{
	cord.X = x;
	cord.Y = y;
...
void addrecord()
{
	...
	while (another == 'y') {
		...
		fwrite(&e, size, 1, fp);//taken input writes the whole thing to file

		printf("\nWant to add another"
			" record (Y/N) : ");
		fflush(stdin);//gives better output
...
void deleterecord()
{
	...
	while (another == 'y') {
		...
		ft = fopen("temp.txt", "wb");//writing in binary-mode that we used
		rewind(fp);
